...
template <class Real> void test(sctl::Comm comm) {
  ...
  // // std::tuple<sctl::SlenderElemList<Real>,sctl::Vector<Real>> build0 = obj.build_straight(Nelem_channel, ElemOrder, FourierOrder, 0, 1, 0.2, comm, ptcls, ptcls_rs, ptcls_Xcs, 0);
  // std::tuple<sctl::SlenderElemList<Real>,sctl::Vector<Real>> build0 = obj.build_conv_div(Nelem_channel, ElemOrder, FourierOrder, 0, 1, 0.1, 0.2, comm, ptcls, ptcls_rs, ptcls_Xcs, 1, 0);
  // const auto elem_lst0 = std::get<0>(build0);
  // const auto NormalOrient = std::get<1>(build0);
