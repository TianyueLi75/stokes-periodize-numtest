...
template <class Real> void test(sctl::Long Nelem, sctl::Long FourierOrder, bool write_ref, sctl::Integer peri_mode, sctl::Comm comm, sctl::Long Nptcl, sctl::Long geom_mode, sctl::Long Ncopy, sctl::Long precond_mode) {
    ...
    Nptcl = ptcls_rs.Dim(); 
    // std::cout << "periodic mode is " << peri_mode << ", Nrepeat is " << Nrepeat << ", Number of particle is " << Nptcl << std::endl;
    ...
    // elem_lst0.WriteVTK("vis/spheroids",X0,comm);
...
template <class Real> void test_NpNf_grid(sctl::Long Nelem, sctl::Long FourierOrder, sctl::Integer peri_mode, sctl::Comm comm, sctl::Long Nptcl, sctl::Long geom_mode, sctl::Long Ncopy) {

    if (!comm.Rank()) {
        std::cout << "Nelem = " << Nelem << ", Fourier order = " << FourierOrder << std::endl;
    }
    // Combine single-layer and double-layer kernels in these proportions
    const Real SL_scal = 1.0;
    const Real DL_scal = 1.0;
    ...
    if (FourierOrder <=8 && Nelem <4) {
        gmres_tol = 1e-4;
        tol = 1e-10;
    } else if (FourierOrder <= 16) {
        gmres_tol = 1e-6;
        tol = 1e-10;
    } else if (FourierOrder <= 32) {
        gmres_tol = 1e-10;
        tol = 1e-10;
    } else {
        gmres_tol = 1e-12;
