...
int main(int argc, char** argv) {
  ...
  using Real = sctl::QuadReal;

    {
      ...
      // Set ProxySurf
      sctl::Vector<Real> X_proxy = Periodize1D<Real>::GetProxySurf_QuadReal(level_in, m0_in); // proxy points coordinates
      ...
      Periodize1D<Real>::EvalFarField_QuadReal(U_far, X0, U_proxy, level_in, m0_in);
