...
template <class Real> class PeriodizeOp {
  ...
  public:
    ...
    static constexpr sctl::Long Nsurf_QuadReal(const sctl::Long m0_in) {
      return 6*(m0_in-1)*(m0_in-1)+2;
    }
    static sctl::Vector<Real> uc_surf_QuadReal(const Real box_length=1, const sctl::Vector<Real>& X0 = sctl::Vector<Real>{0,0,0}, const sctl::Long m0_in=20) {
      return proxy_surf_QuadReal(box_length*2.95, X0, m0_in);
    }
    static sctl::Vector<Real> ue_surf_QuadReal(const Real box_length=1, const sctl::Vector<Real>& X0 = sctl::Vector<Real>{0,0,0}, const sctl::Long m0_in=20) {
      return proxy_surf_QuadReal(box_length*1.05, X0, m0_in);
    }
    static sctl::Vector<Real> dc_surf_QuadReal(const Real box_length=1, const sctl::Vector<Real>& X0 = sctl::Vector<Real>{0,0,0}, const sctl::Long m0_in=20) {
      return proxy_surf_QuadReal(box_length*1.05, X0, m0_in);
    }
    static sctl::Vector<Real> de_surf_QuadReal(const Real box_length=1, const sctl::Vector<Real>& X0 = sctl::Vector<Real>{0,0,0}, const sctl::Long m0_in=20) {
      return proxy_surf_QuadReal(box_length*2.95, X0, m0_in);
    }
    ...
    static std::tuple<sctl::Matrix<Real>, sctl::Matrix<Real>> UC2UE_QuadReal(const sctl::Long m0_in, const Real box_length=1) {
      sctl::Profile::Scoped prof(__FUNCTION__);
      const auto Xc = uc_surf_QuadReal(box_length, sctl::Vector<Real>{0,0,0}, m0_in);
      const auto Xe = ue_surf_QuadReal(box_length, sctl::Vector<Real>{0,0,0}, m0_in);
      const KerM2M ker_m2m;
    ...
    static std::tuple<sctl::Matrix<Real>, sctl::Matrix<Real>> DC2DE_QuadReal(const sctl::Long m0_in, const Real box_length=1) {
      sctl::Profile::Scoped prof(__FUNCTION__);
      const auto Xc = dc_surf_QuadReal(box_length, sctl::Vector<Real>{0,0,0}, m0_in);
      const auto Xe = de_surf_QuadReal(box_length, sctl::Vector<Real>{0,0,0}, m0_in);
      const KerL2L ker_l2l;
    ...
    static sctl::Matrix<Real> BC_UE2DC_QuadReal(const sctl::Long level_in, const sctl::Long m0_in) {
      const auto M = BC_UE2DC_helper_QuadReal(level_in, m0_in);
      return M;
    }
  ...
  private:
    ...
    static sctl::Vector<Real> proxy_surf_QuadReal(const Real box_length, const sctl::Vector<Real>& Xc, const sctl::Long m0_in) {
      // static const sctl::Vector<Real> X0 = []() {
      sctl::Vector<Real> X0;
      for (sctl::Long i0 = 0; i0 < m0_in; i0++) {
        for (sctl::Long i1 = 0; i1 < m0_in; i1++) {
          for (sctl::Long i2 = 0; i2 < m0_in; i2++) {
            if (i0==0 || i0==m0_in-1 || i1==0 || i1==m0_in-1 || i2==0 || i2==m0_in-1) {
              const Real x = i0/(Real)(m0_in-1);
              const Real y = i1/(Real)(m0_in-1);
              const Real z = i2/(Real)(m0_in-1);
              X0.PushBack(x-0.5);
              X0.PushBack(y-0.5);
              X0.PushBack(z-0.5);
            }
      ...
      // }();
      SCTL_ASSERT(X0.Dim() == Nsurf_QuadReal(m0_in)*COORD_DIM);

      sctl::Vector<Real> X(Nsurf_QuadReal(m0_in)*COORD_DIM);
      for (sctl::Long i = 0; i < Nsurf_QuadReal(m0_in); i++) {
        for (sctl::Long k = 0; k < COORD_DIM; k++) {
          X[i*COORD_DIM+k] = X0[i*COORD_DIM+k]*box_length + Xc[k];
        }
      }
    ...
    static sctl::Matrix<Real> BC_UE2DC_helper_QuadReal(const sctl::Long level_in, const sctl::Long m0_in) {
      sctl::Profile::Scoped prof(__FUNCTION__);

      std::string data_file;
      sctl::Matrix<Real> M;
      data_file = "data/Mbc_ue2dc_1d_l"+std::to_string(level_in)+"_m"+std::to_string(m0_in)+"_float128.mat";
      M.template Read<sctl::QuadReal>(data_file.c_str());
      if (M.Dim(0) || M.Dim(1)) return M;

      const KerM2L ker_m2l;
      const sctl::Integer kdim[2] = {KerM2L::SrcDim(), KerM2L::TrgDim()};
      const auto X0 = dc_surf_QuadReal(1,sctl::Vector<Real>{0,0,0}, m0_in);

      sctl::Matrix<Real> M2M(Nsurf_QuadReal(m0_in)*kdim[0], Nsurf_QuadReal(m0_in)*kdim[0]);
      M.ReInit(Nsurf_QuadReal(m0_in)*kdim[0], Nsurf_QuadReal(m0_in)*kdim[1]);
      M2M.SetZero();
      M.SetZero();
      ...
      for (sctl::Long l = 0; l < level_in; l++) { // tree-code (hierarchical) summation
        std::cout<<"level = "<<l<<'\n';
        const sctl::Long box_length = ((sctl::Long)1) << l;

        sctl::Matrix<Real> M0, M1;
        ker_m2l.KernelMatrix<Real,true>(M0, X0, ue_surf_QuadReal(box_length, sctl::Vector<Real>{ (Real)1.5*box_length+0.5,0,0},m0_in), sctl::Vector<Real>());
        ker_m2l.KernelMatrix<Real,true>(M1, X0, ue_surf_QuadReal(box_length, sctl::Vector<Real>{-(Real)1.5*box_length-0.5,0,0},m0_in), sctl::Vector<Real>());
        M += M2M * (M0 + M1);

        ker_m2l.KernelMatrix<Real,true>(M0, uc_surf_QuadReal(2*box_length,sctl::Vector<Real>{0,0,0},m0_in), ue_surf_QuadReal(box_length, sctl::Vector<Real>{ (Real)0.5*box_length,0,0},m0_in), sctl::Vector<Real>());
        ker_m2l.KernelMatrix<Real,true>(M1, uc_surf_QuadReal(2*box_length,sctl::Vector<Real>{0,0,0},m0_in), ue_surf_QuadReal(box_length, sctl::Vector<Real>{-(Real)0.5*box_length,0,0},m0_in), sctl::Vector<Real>());
        const auto [M_uc2ue0, M_uc2ue1] = PeriodizeOp<Real>::UC2UE_QuadReal(m0_in, 2*box_length);
        M2M = M2M * (((M0+M1) * M_uc2ue0) * M_uc2ue1);
      }
      //for (sctl::Long i = -1000; i <= 1000; i++) { // direct summation
      //  if (abs(i) >= 2) {
...
template <class Real> sctl::Vector<Real> Periodize1D<Real>::GetProxySurf(const sctl::Long m0_in) {
  ...
  std::string data_dn = "data/dn_equiv_surf_1d_m"+std::to_string(m0_in)+".mat";
  sctl::Vector<Real> X;
  X.template Read<Real>(data_dn.c_str());
  if (!X.Dim()) {
    std::cout << "writing equiv surf to file." << std::endl;
    X = PeriodizeOp<Real>::uc_surf(1, sctl::Vector<Real>{0.5,0.5,0.5});
    X.template Write<Real>(data_dn.c_str());
  } 
  return X;
  // }();
  // return proxy_surf;
...
template <class Real> sctl::Vector<Real> Periodize1D<Real>::GetProxySurf_QuadReal(const sctl::Long m0_in) {
  // sctl::Vector<Real> proxy_surf = [&level_in,&m0_in](){
  sctl::Vector<Real> X;
  std::string data_dn = "data/dn_equiv_surf_1d_m"+std::to_string(m0_in)+"_float128.mat";
  X.template Read<sctl::QuadReal>(data_dn.c_str());
  if (!X.Dim()) {
    std::cout << "writing equiv surf to file." << std::endl;
    X = PeriodizeOp<Real>::uc_surf_QuadReal(1, sctl::Vector<Real>{0.5,0.5,0.5}, m0_in);
...
template <class Real> void Periodize1D<Real>::EvalFarField_QuadReal(sctl::Vector<Real>& U_far, const sctl::Vector<Real>& Xt, const sctl::Vector<Real>& U_proxy, const sctl::Long level_in, const sctl::Long m0_in) {
  // std::cout << "in eval far field, getting matrices. " << std::endl;
  const auto Mbc0 = GetMat_UC2DE0_QuadReal(level_in, m0_in);
  const auto Mbc1 = GetMat_UC2DE1_QuadReal(level_in, m0_in);
  const sctl::Long N = Mbc0.Dim(0);
  SCTL_ASSERT(U_proxy.Dim() == N);
  ...
  U_far = 0;
  static const sctl::Stokes3D_FxU stokeslet;
  stokeslet.template Eval<Real,true>(U_far, Xt, GetProxySurf_QuadReal(m0_in), sctl::Vector<Real>(), sctl::Vector<Real>(N,proxy_density.begin(),false));
}
...
template <class Real> sctl::Matrix<Real> Periodize1D<Real>::GetMat_UC2DE0_QuadReal(const sctl::Long level_in, const sctl::Long m0_in) {
  // sctl::Matrix<Real> Mbc = [&level_in,&m0_in](){
  sctl::Matrix<Real> M;
  std::string data_file = "data/Mbc_uc2de0_1d_l"+std::to_string(level_in)+"_m"+std::to_string(m0_in)+"_float128.mat";
  M.template Read<sctl::QuadReal>(data_file.c_str());
  if (M.Dim(0) || M.Dim(1)) return M;

  std::cout << " read file unsuccessful, making UC2DE0 mats <QuadReal> now." << std::endl;
  const auto [M_uc2ue0, M_uc2ue1] = PeriodizeOp<Real>::UC2UE_QuadReal(m0_in);
  const auto [M_dc2de0, M_dc2de1] = PeriodizeOp<Real>::DC2DE_QuadReal(m0_in);
  const auto Mbc_ue2dc = PeriodizeOp<Real>::BC_UE2DC_QuadReal(level_in,m0_in);
  M = (M_uc2ue0 * (M_uc2ue1 * Mbc_ue2dc)) * M_dc2de0;
  M.template Write<sctl::QuadReal>(data_file.c_str());
...
template <class Real> sctl::Matrix<Real> Periodize1D<Real>::GetMat_UC2DE1(const sctl::Long level_in, const sctl::Long m0_in) {
  ...
  M_dc2de1.template Read<Real>(data_file.c_str());
  if (M_dc2de1.Dim(0)==0 || M_dc2de1.Dim(1)==0) {
    std::cout << "Writing data file for uc2de1 <double>" << std::endl;
    std::tuple<sctl::Matrix<Real>, sctl::Matrix<Real>> tpl2 = PeriodizeOp<Real>::DC2DE();
    M_dc2de1 = std::get<1>(tpl2);
    M_dc2de1.template Write<Real>(data_file.c_str());
  }
  return M_dc2de1;
  // }();
  // return Mbc;
...
template <class Real> sctl::Matrix<Real> Periodize1D<Real>::GetMat_UC2DE1_QuadReal(const sctl::Long level_in, const sctl::Long m0_in) {
  // sctl::Matrix<Real> Mbc = [&level_in,&m0_in](){
  std::string data_file = "data/M_dc2de1_1d_m"+std::to_string(m0_in)+"_float128.mat";
  sctl::Matrix<Real> M;
  M.template Read<sctl::QuadReal>(data_file.c_str());
  if (M.Dim(0) || M.Dim(1)) return M;
  
  std::cout << "Writing data file for uc2de1 <QuadReal>" << std::endl;
  const auto [M_dc2de0, M_dc2de1] = PeriodizeOp<Real>::DC2DE_QuadReal(m0_in);
